...
void startColorSensor(){
  ...
  if (tcs.begin()) {
    ...
    colorValues.data_length = COLORDATALENGTH;
...
void getColor(){
  ...
  // Map color values between 0 and 255 depending on the clear value and save them
  colorValues.data[0] = (std::min<unsigned int>)((red*1.0 / clear) * 256, 255);
  colorValues.data[1] = (std::min<unsigned int>)((green*1.0 / clear) * 256, 255);
  colorValues.data[2] = (std::min<unsigned int>)((blue*1.0 / clear) * 256, 255);
  Serial.print(colorValues.data[0]);
  Serial.print("\t");
  Serial.println(colorValues.data[1]);
}
...
void setup()
{
  ...
  nh.subscribe(get_color_value_sub);
  ...
  nh.advertise(color_sensor_value_pub);
...
void loop()
{
  ...
  if ((t-tTime[7]) >= (1000 / GET_COLOR_VALUE_FREQUENCY))
  {
    if (doGetColor) {
      //Read the color 
      getColor();
      //Publish the color
      color_sensor_value_pub.publish( &colorValues );
    }
    tTime[7] = t;
  }
...
/*******************************************************************************
* Callback function for get_color msg
*******************************************************************************/
void getColorCallback(const std_msgs::Bool& get_color_msg)
{
  extern bool doGetColor;
  doGetColor = get_color_msg.data;
}
